...
for level in range(num_levels):
    ...
    print("level: %d\tsample mean: %.2e\ttime: %.1e\t time ratio: %.1f"%(level,solutions[level].mean(),times[level],-1 if level==0 else times[level]/times[level-1]))
...
fig.tight_layout()
fig.savefig("dakota_ssd1_scratch.hists.png")

fig,ax = plt.subplots(1,1,figsize=(8,5))
ax.plot(times)
ax.set_yscale("log")
fig.tight_layout()
fig.savefig("dakota_ssd1_scratch.times.png")