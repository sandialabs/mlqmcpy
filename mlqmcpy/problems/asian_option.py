...
def standard_brownian_motion_kl(t, x, t_final=1):
    ...
    assert isinstance(t,np.ndarray) and t.ndim==1 and (0<=t).all() and (t<=t_final).all()
    assert isinstance(x,np.ndarray) and x.ndim>=1
    t01 = t/t_final
    z = norm.ppf(x)
    j = np.arange(z.shape[-1])
    factor = 2/(2*j+1)
    sbm01 = (np.sqrt(2)/np.pi*z[...,None]*factor[...,None]*np.sin(np.pi*t01/factor[...,None])).sum(-2)
...
def asian_option(
    ...
    """
    if coeffs is None: coeffs = np.random.rand(8)
