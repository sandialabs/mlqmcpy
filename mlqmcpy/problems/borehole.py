...
def borehole(level, samples=None):
    ...
    distribs = [
        ...
        scipy.stats.uniform(loc=1500,scale=15000-1500), # scipy.stats.uniform(loc=9855,scale=12045-9855)
    ]
    assert samples.shape[-1]==8 
    assert level in [0,1], "borehole only supports level=0 (low fidelity) or level=1 (high fidelity)"
